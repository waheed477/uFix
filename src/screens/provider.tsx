...
function RequestCard({ req, onSend }: { req: IncomingRequest; onSend: (charge: number) => void }) {
  ...
  return (
    <div className="animate-slide-up rounded-2xl bg-white p-4 shadow-card">
      ...
      <div className="mt-3 flex items-center gap-2 border-t border-ink-100 pt-3">
        ...
        <Button size="sm" className="h-11 flex-1" onClick={() => onSend(charge)}>
...
export function ProviderHome() {
  const { user, nearbyRequests, toggleOnline, sendOffer, jobs } = useApp();
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="leading-tight">
              <p className="text-xs font-medium text-ink-500">Welcome back 👋</p>
        ...
        {!online ? (
          <div className="flex flex-col items-center justify-center rounded-3xl border-2 border-dashed border-ink-200 bg-white px-8 py-14 text-center">
            ...
            <p className="mt-1 max-w-[240px] text-sm text-ink-500">
              Flip the switch above to go online and start receiving requests in real time.
            </p>
          </div>
          ...
          <div className="flex flex-col items-center justify-center rounded-3xl bg-white px-8 py-14 text-center shadow-card">
            ...
            <p className="mt-1 max-w-[240px] text-sm text-ink-500">
              No pending requests right now. New jobs will appear here the moment customers post them.
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            {nearbyRequests.map((r) => (
              <RequestCard key={r.id} req={r} onSend={(charge) => sendOffer(r.id, charge)} />
            ))}
          </div>
        )}
      </div>
